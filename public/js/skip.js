...
function verifySkipRequest(requestedBotName, requestedBranchName, requestedEdgeName, requestedBranchCl, requestedBranchData) {
    ...
    if (!requestedEdgeData) {
        skipFailure(`${requestedBotName}:${requestedBranchName} has no edge for "${requestedEdgeName}".`)
