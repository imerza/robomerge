...
import { GetStompVerificationResult, MergeConflictAdditionalInformation, MergeAction, OperationResult, PendingChange }  from './branch-interfaces';
import { resolveBranchArg, ResolveResultAdditionalInfo, StompedRevision, StompVerification, StompVerificationFile, UnlockVerification }  from './branch-interfaces';
...
export class NodeBot extends PerforceStatefulBot implements NodeBotInterface {
	...
	private stompFailureResult(message: string): StompVerification {
		return {
			success: false,
			message,
	...
	async verifyStomp(changeCl: number, targetBranchName: string) : Promise<StompVerification> {
		...
		if (!changeCl || !targetBranchName) {
			return this.stompFailureResult("Missing parameters for stomp node operation")
		}

		const targetBranch = this._getBranch(targetBranchName)

		if (!targetBranch) {
			return this.stompFailureResult(`Unable to retrieve branch infomation for "${targetBranchName}". Unable to stomp, please contact Robomerge support."`)
		}

		const edge = this.getImmediateEdge(targetBranch)

		if (!edge) {
			return this.stompFailureResult(`This bot has no edge for specified branch "${targetBranch.name}" on ${this.fullName}`)
		}

		// Ensure we're paused on a non-manual pause blockage
		if (!edge.isBlocked) {
			return this.stompFailureResult("Branch needs to have conflict to stomp changes")
		}
		...
		// We've ensured we're on a blockage now. Make sure the CL is still valid.
		if (blockageInfo.change !== changeCl) {
			return this.stompFailureResult(`Requested change CL "${changeCl}" does not match blockage change CL "${blockageInfo.change}"`)
		}

		let conflict = this.conflicts.getConflictByChangeCl(changeCl, targetBranch)

		if (!conflict) {
			return this.stompFailureResult(`Unable to retrieve conflict for CL ${changeCl} merging to ${targetBranch.name}. Unable to stomp, please contact Robomerge support.`)
		}
		if (conflict.kind === 'Exclusive check-out') {
			return this.stompFailureResult(`Unable to stomp changes over exclusive checkout to files in ${targetBranch.name}. ` + 
				`Unable to stomp, please revert the locked files in ${targetBranch.name} or contact Robomerge support.`)
		}

		return this._evaluateConflictForStomp(changeCl, targetBranch, blockageInfo.additionalInfo)
	}

	async _evaluateConflictForStomp(changeCl: number, targetBranch: Readonly<EditableBranch>, mergeConflictInfo: MergeConflictAdditionalInformation) : Promise<StompVerification>
	{
		try {
			// Get what is left to resolve out of the changelist
			const resolveResult = Object.assign(new ResolveResultAdditionalInfo(), mergeConflictInfo.resolveResult)

			if (!resolveResult.hasConflict()) {
				return this.stompFailureResult(`No conflict found resolving CL ${changeCl}`)
			}
			if (!resolveResult.successfullyParsedConflicts) {
				return this.stompFailureResult(`Encountered error enumerating conflicting files after integration attempt: ${resolveResult.parsingError}`)
			}
			...
			for (const remainingFile of remainingFiles) {
				// Find target file 
				const targetFileInDepot = await this.p4.where(mergeConflictInfo.client, remainingFile.clientFile)
			...
			for (const file of mergeConflictInfo.describeResult.entries) {
				...
				if (!resolved) {
					...
					if (file.depotFile.startsWith('//Tasks/')) {
						...
						const setDebugScope = (debugScope : Sentry.Scope) => {
							...
							debugScope.setExtra('resolveResult', resolveResult!.conflictsText)
							debugScope.setExtra('resolveResultParsingError?', resolveResult!.parsingError)
						}
			...
			return this.stompFailureResult(`Encountered error processing ${changeCl}: ${error}`)
		} 
	}
	...
	async verifyUnlock(changeCl: number, targetBranchName: string) : Promise<UnlockVerification> {
		...
		if (lockedFiles.length > RoboArgs.getExclusiveLockOpenedsToRun()) {
			lockedFiles = [...lockedFiles.slice(0,RoboArgs.getExclusiveLockOpenedsToRun()), ...(await this.p4.getOpenedDetails(lockedFiles.slice(RoboArgs.getExclusiveLockOpenedsToRun())))]
		}
	...
	async applyStatus(status: BranchStatus) {
		...
		await this.conflicts.applyStatus(status.conflicts, this.slackMessages)
		...
		if (info.targets && availableEdges.size > 0) {
			...
			for (const action of info.targets) {
				if (action.mergeMode === 'normal' && !action.flags.has('disregardassetblock')) {
					...
					for (const [blockTargetName, blockType] of this.branch.blockAssetTargets) {
						if (this._getBranch(blockTargetName) === action.branch) {
							...
							const autoFlow = (blockType == 'all' && info.cl > edge.lastCl)
							if (autoFlow || (info.userRequest && (blockType == 'all' || (!info.forceStompChanges && !info.forceCreateAShelf)))) {
	...
	async handleMergeFailure(failure: Failure, pending: PendingChange, postIntegrate?: boolean) {
		...
		if (pending.change.userRequest) {
			...
			if (this.slackMessages) {
				...
				let emailAddress = await this.findEmail(owner)
				await this.slackMessages.postDM(emailAddress, pending.newCl, this.branch, dm)
