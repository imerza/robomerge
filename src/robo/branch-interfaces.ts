...
import { BranchSpec, ConflictedResolveNFile, DescribeResult, EdgeServer, ExclusiveFileDetails, ResolveResult } from '../common/perforce';
...
export function GetStompVerificationResult(verifyResult: StompVerification): OperationResult {
...
export type BlockAssetType = 'all' | 'reconsider'
export type EditableBranch = BranchBase & {
	...
	blockAssetTargets: Map<string, BlockAssetType>
	graphFilteredTargets: Set<string>
...
export class ResolveResultAdditionalInfo {

	protected remainingConflicts: ConflictedResolveNFile[]
	readonly conflictsText: string
	readonly hasUnknownConflicts: boolean
	readonly parsingError: string = ""
	readonly successfullyParsedConflicts: boolean

	constructor(resolveResult?: ResolveResult) {
		if (resolveResult) {
			this.conflictsText = resolveResult.getConflictsText()
			this.hasUnknownConflicts = resolveResult.getHasUnknownConflicts()
			this.parsingError = resolveResult.getParsingError()
			this.remainingConflicts = resolveResult.getConflicts()
			this.successfullyParsedConflicts = resolveResult.successfullyParsedConflicts()
		}
	}
...
export interface MergeConflictAdditionalInformation {
	resolveResult: ResolveResultAdditionalInfo,
