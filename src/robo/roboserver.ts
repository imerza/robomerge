...
class RoboWebApp implements AppInterface {
	...
	@Handler('GET', '/helpPageURL')
	async helpPageURL() {
		return ENVIRONMENT.helpPageURL
	}
	...
	async getWorkspaces(serverID: string): Promise<OperationReturnType> {
		if (!this.authData) {
			throw new Error('Secure call but no auth data?')
		}

		return await this.sendMessage('getWorkspaces', [this.authData.user, serverID])
	}
