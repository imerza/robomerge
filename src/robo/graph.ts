...
import { matchPrefix, setDefault, sortBy } from '../common/helper';
...
export class Graph {
	...
	findNodeForStream(depotPath: string | Branch, partialMatchPath?: string) {
		const branchId = makeBranchId(depotPath)
		let nodeForStream = this.streamNodes.get(branchId)
		if (!nodeForStream && partialMatchPath) {

			// don't match unless there is a match of at least as long as the root depotPath
			const minMatchLength = (branchId as string).length
			let target_match = minMatchLength-1
			for (const [stream, node] of this.streamNodes) {
				const matchlen = matchPrefix(stream.toLowerCase(), partialMatchPath.toLowerCase())
