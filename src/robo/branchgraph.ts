...
import { setDefault, setDifference, setIntersection, setUnion } from '../common/helper';
import { BranchSpec } from '../common/perforce';
import { BlockAssetType, Branch, BranchGraphInterface, EditableBranch } from './branch-interfaces';
...
if (botname === '__TEST__') {
	...
	private add(name: string, options: NodeOptions) {
		...
		let branch: EditableBranch = {
			...
			blockAssetTargets: new Map<string, BlockAssetType>(),
			graphFilteredTargets: new Set<string>(),
		...
		const processBranchArray = function(arr: string[], propName: string) {
			...
			if (duplicateValues.length > 0) {
				throw new Error(`Found duplicate entries [${duplicateValues.join(', ')}] in ${propName}`)
			}
			return uniqueValues
		}

		const FlowsToBranches = processBranchArray(branch.flowsTo, 'flowsTo')

		const checkMissingFlowsTo = function(branches: Set<string>, msgPrefix: string) {
			const MissingFlowsTo = setDifference(branches, FlowsToBranches)
			if (MissingFlowsTo.size == 1) {
				throw new Error(`${msgPrefix} '${[...MissingFlowsTo][0]}' not in flowsTo list`)
			} else if (MissingFlowsTo.size > 1) {
				throw new Error(`${msgPrefix}s [${[...MissingFlowsTo].join(', ')}] not in flowsTo list`)
			}
		}

		let BlockAssetFlowBranches: Set<string>|null = null
		if (options.blockAssetFlow) {
			BlockAssetFlowBranches = processBranchArray(options.blockAssetFlow, 'blockAssetFlow')
			checkMissingFlowsTo(BlockAssetFlowBranches, "Block asset flow")
			BlockAssetFlowBranches.forEach(v => branch.blockAssetTargets.set(v, 'all'))
		}

		if (options.blockAssetReconsider) {
			const BlockAssetReconsiderBranches = processBranchArray(options.blockAssetReconsider, 'blockAssetReconsider')
			checkMissingFlowsTo(BlockAssetReconsiderBranches, "Block asset reconsider")
			if (BlockAssetFlowBranches) {
				const DuplicateBlockAssets = setIntersection(BlockAssetReconsiderBranches, BlockAssetFlowBranches)
				if (DuplicateBlockAssets.size == 1) {
					throw new Error(`Block asset reconsider '${[...DuplicateBlockAssets][0]}' already in block asset flow list`)
				} else if (DuplicateBlockAssets.size > 1) {
					throw new Error(`Block asset reconsiders [${[...DuplicateBlockAssets].join(', ')}] already in block asset flow list`)
				}
			}
			BlockAssetReconsiderBranches.forEach(v => branch.blockAssetTargets.set(v, 'reconsider'))
		}

		if (options.graphFilteredEdges) {
			branch.graphFilteredTargets = processBranchArray(options.graphFilteredEdges, 'graphFilteredEdges')
			checkMissingFlowsTo(branch.graphFilteredTargets, "Graph filtered edge")
		}

		const DefaultFlowBranches = processBranchArray(branch.defaultFlow, 'defaultFlow')

		// apply forced merge targets
		{
			const forceFlow = (options.forceAll ? options.flowsTo : options.forceFlowTo) || []
			const ForceFlowBranches = processBranchArray(forceFlow, 'forceFlowTo')
			const MissingFlowsTo = setDifference(ForceFlowBranches, FlowsToBranches)
			if (MissingFlowsTo.size == 1) {
				throw new Error(`Cannot force ${branch.name} to flow to '${[...MissingFlowsTo][0]}' because it is not in the flowsTo list`)
			} else if (MissingFlowsTo.size > 1) {
				throw new Error(`Cannot force ${branch.name} to flow to [${[...MissingFlowsTo].join(', ')}] because they are not in the flowsTo list`)
			}
			branch.forceFlowTo = [...ForceFlowBranches]
			branch.defaultFlow = [...setUnion(ForceFlowBranches, DefaultFlowBranches)]
		}
		...
		// add to names
		this.names.set(nameUpper, branch)
